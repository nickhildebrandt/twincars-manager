<script lang="ts">
  ...
  const initial = await untrack(() => listInvoicesRemote(queryArgs))
  ...
  const result = $derived.by(
    () => listInvoicesRemote(queryArgs).current ?? lastResult
  )
  ...
  $effect(() => {
    const query = listInvoicesRemote(queryArgs)
    ...
    try {
      ...
      await busy.run(() =>
        deleteInvoiceRemote({ id }).updates(
          listInvoicesRemote(queryArgs).withOverride((current) => ({
            ...current,
          ...
          <li class="hover:bg-base-200 flex items-stretch gap-2 p-3">
            ...
            >
              ...
              <span class="text-base-content/70 truncate text-xs">
                {[i.issueDate, i.customerName].filter(Boolean).join(' · ')}
              </span>
              <span class="mt-0.5 flex flex-wrap items-center gap-2">
                ...
                <span class="font-mono text-sm">
                  {formatEuro(Number(i.grossTotal))}
                </span>
                {#if i.totalPaid > 0}
                  <span class="text-base-content/60 text-xs">
                    Bezahlt: {formatEuro(i.totalPaid)}
                  </span>
                {/if}
              </span>
            </a>
