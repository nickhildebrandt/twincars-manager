---
title: GET /api/pickers/items
kategorie: api
method: GET
path: /api/pickers/items
permission: items, orders
status: umgesetzt
features: []
...
# GET /api/pickers/items

Die Auswahl für Artikel und Leistungen: serverseitig gesucht, serverseitig geblättert, fest
25 je Seite.
...
`items, orders` — die Absage nennt den Bereich auf Deutsch:
„Sie haben keine Berechtigung für Leistungen und Artikel oder Aufträge."
...
| `q` | nein | Suchbegriff; gesucht wird über Bezeichnung und Artikelnummer |
...
Die Unterzeile trägt den **heute gültigen** Preis, gezogen in derselben Abfrage. Ein Preis, der erst ab einem künftigen Datum gilt, erscheint nicht.
...
| 403 | Recht fehlt | Sie haben keine Berechtigung für Leistungen und Artikel oder Aufträge. |
...
Dienst dahinter heißt `pickItems` in `server/services/picker-service.ts`.
