---
title: GET /api/pickers/employees
kategorie: api
method: GET
path: /api/pickers/employees
permission: employees, orders
status: umgesetzt
features: []
...
# GET /api/pickers/employees

Die Auswahl für Mitarbeiter: serverseitig gesucht, serverseitig geblättert, fest
25 je Seite.
...
`employees, orders` — die Absage nennt den Bereich auf Deutsch:
„Sie haben keine Berechtigung für Mitarbeiter oder Aufträge."
...
| `q` | nein | Suchbegriff; gesucht wird über Nachname, Vorname und Personalnummer |
...
Deaktivierte Mitarbeiter erscheinen nicht (M-12). Sie bleiben überall stehen, wo sie schon eingetragen sind — nur neu ausgewählt werden sie nicht mehr.
...
| 403 | Recht fehlt | Sie haben keine Berechtigung für Mitarbeiter oder Aufträge. |
...
Dienst dahinter heißt `pickEmployees` in `server/services/picker-service.ts`.
