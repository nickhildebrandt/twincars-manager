---
id: F-457
title: Neue Buchung anlegen
status: geplant
modul: Buchhaltung, Kategorien, DATEV-Export
paket: T-028
permission: offen
routes: ['/ledger/new']
endpoints: ['createLedgerEntryRemote', 'listCategoriesRemote']
tables: ['ledger_entries', 'ledger_categories']
schemas: []
components: []
tests: []
updated: 2026-09-13
---

# F-457 — Neue Buchung anlegen

> **Status: geplant.** Diese Seite hält das Zielverhalten fest, wie es im
> Inventar des Vorgängersystems erhoben wurde. Sie wird mit Arbeitspaket
> **T-028** ausgefüllt und auf `status: umgesetzt` gesetzt.

## Zweck

Neue Buchung anlegen

## Erwartetes Verhalten

Felder Art (Default Ausgabe), Datum (Preset aus `?date=`, sonst heute), Beschreibung, Kategorie, Steuersatz 19/7/0, Brutto, Zahlungsart, Zahlungsstatus (Default Bezahlt); `source='manual'`; Erfolg → Toast "Buchung gespeichert." + `/ledger`

## Nutzersicht

_Wird mit T-028 ergänzt: was der Bediener sieht und tut, Schritt für Schritt._

## Berechtigungen

_Wird mit T-028 ergänzt._

## Zustände

| Zustand | Verhalten |
| --- | --- |
| Leer | _offen_ |
| Laden | _offen_ |
| Fehler | _offen_ |
| Keine Berechtigung | _offen_ |

## Technischer Bezug

| | |
| --- | --- |
| Routen | `/ledger/new` |
| Endpoints | `createLedgerEntryRemote`, `listCategoriesRemote` |
| Tabellen | `ledger_entries`, `ledger_categories` |
| Schemata | _offen_ |
| Tests | _offen_ |

## Bekannte Grenzen

_Wird mit T-028 ergänzt._

## Quellen

- Inventar: [F-457 in 01-inventar.md](../rewrite/01-inventar.md)
- Arbeitspaket: [T-028 in 06-arbeitsplan.md](../rewrite/06-arbeitsplan.md)
- Übersicht: [docs/index.md](../index.md)
