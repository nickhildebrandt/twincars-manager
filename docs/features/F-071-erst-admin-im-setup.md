---
id: F-071
title: Erst-Admin im Setup
status: geplant
modul: Authentifizierung, Berechtigungen, Benutzer und Rollen
paket: T-010
permission: offen
routes: ['/setup']
endpoints: ['createInitialAdmin']
tables: ['users', 'accounts', 'user_roles']
schemas: []
components: []
tests: []
updated: 2026-09-13
---

# F-071 — Erst-Admin im Setup

> **Status: geplant.** Diese Seite hält das Zielverhalten fest, wie es im
> Inventar des Vorgängersystems erhoben wurde. Sie wird mit Arbeitspaket
> **T-010** ausgefüllt und auf `status: umgesetzt` gesetzt.

## Zweck

Erst-Admin im Setup

## Erwartetes Verhalten

nur solange kein Benutzer existiert; Rolle Administrator automatisch; Username lowercase

## Nutzersicht

_Wird mit T-010 ergänzt: was der Bediener sieht und tut, Schritt für Schritt._

## Berechtigungen

_Wird mit T-010 ergänzt._

## Zustände

| Zustand | Verhalten |
| --- | --- |
| Leer | _offen_ |
| Laden | _offen_ |
| Fehler | _offen_ |
| Keine Berechtigung | _offen_ |

## Technischer Bezug

| | |
| --- | --- |
| Routen | `/setup` |
| Endpoints | `createInitialAdmin` |
| Tabellen | `users`, `accounts`, `user_roles` |
| Schemata | _offen_ |
| Tests | _offen_ |

## Bekannte Grenzen

_Wird mit T-010 ergänzt._

## Quellen

- Inventar: [F-071 in 01-inventar.md](../rewrite/01-inventar.md)
- Arbeitspaket: [T-010 in 06-arbeitsplan.md](../rewrite/06-arbeitsplan.md)
- Übersicht: [docs/index.md](../index.md)
