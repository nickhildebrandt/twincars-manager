...
Ein globaler `busy`-Store: alle Mutationen in `busy.run`, Buttons `disabled={busy.active}` mit Spinner; Kanban SSR via top-level await; Detail lädt drei Queries parallel
...
- Inventar: [F-362 in 01-inventar.md](../rewrite/01-inventar.md)
