---
id: F-468
title: Standard-Kategorien (Seed)
status: geplant
modul: Buchhaltung, Kategorien, DATEV-Export
paket: T-028
permission: offen
routes: []
endpoints: ['seedDefaults()']
tables: ['ledger_categories']
schemas: []
components: []
tests: []
updated: 2026-09-13
---

# F-468 — Standard-Kategorien (Seed)

> **Status: geplant.** Diese Seite hält das Zielverhalten fest, wie es im
> Inventar des Vorgängersystems erhoben wurde. Sie wird mit Arbeitspaket
> **T-028** ausgefüllt und auf `status: umgesetzt` gesetzt.

## Zweck

Standard-Kategorien (Seed)

## Erwartetes Verhalten

13 Kategorien (3 income, 10 expense) beim ersten Request/Setup; idempotent über `name` unique; kein Konto, kein Steuersatz-Default; keine Verwaltungs-UI (Anlegen/Umbenennen/Löschen nicht möglich)

## Nutzersicht

_Wird mit T-028 ergänzt: was der Bediener sieht und tut, Schritt für Schritt._

## Berechtigungen

_Wird mit T-028 ergänzt._

## Zustände

| Zustand | Verhalten |
| --- | --- |
| Leer | _offen_ |
| Laden | _offen_ |
| Fehler | _offen_ |
| Keine Berechtigung | _offen_ |

## Technischer Bezug

| | |
| --- | --- |
| Routen | — |
| Endpoints | `seedDefaults()` |
| Tabellen | `ledger_categories` |
| Schemata | _offen_ |
| Tests | _offen_ |

## Bekannte Grenzen

_Wird mit T-028 ergänzt._

## Quellen

- Inventar: [F-468 in 01-inventar.md](../rewrite/01-inventar.md)
- Arbeitspaket: [T-028 in 06-arbeitsplan.md](../rewrite/06-arbeitsplan.md)
- Übersicht: [docs/index.md](../index.md)
