---
id: F-486
title: Creation-Flow-Leaf 'Mitarbeiter'
status: geplant
modul: Mitarbeiter, Abwesenheiten, Zeiterfassung, Öffnungszeiten
paket: T-017
permission: offen
routes: ['/employees/new']
endpoints: ['createEmployeeRemote']
tables: []
schemas: []
components: []
tests: []
updated: 2026-09-13
---

# F-486 — Creation-Flow-Leaf "Mitarbeiter"

> **Status: geplant.** Diese Seite hält das Zielverhalten fest, wie es im
> Inventar des Vorgängersystems erhoben wurde. Sie wird mit Arbeitspaket
> **T-017** ausgefüllt und auf `status: umgesetzt` gesetzt.

## Zweck

Creation-Flow-Leaf "Mitarbeiter"

## Erwartetes Verhalten

Wenn `creationFlow.top.entity === 'employee'`: Info-Alert, Erfolg → `finish({ id, label })` → `returnUrl`; Abbrechen → `cancel()` → `returnUrl`

## Nutzersicht

_Wird mit T-017 ergänzt: was der Bediener sieht und tut, Schritt für Schritt._

## Berechtigungen

_Wird mit T-017 ergänzt._

## Zustände

| Zustand | Verhalten |
| --- | --- |
| Leer | _offen_ |
| Laden | _offen_ |
| Fehler | _offen_ |
| Keine Berechtigung | _offen_ |

## Technischer Bezug

| | |
| --- | --- |
| Routen | `/employees/new` |
| Endpoints | `createEmployeeRemote` |
| Tabellen | — |
| Schemata | _offen_ |
| Tests | _offen_ |

## Bekannte Grenzen

_Wird mit T-017 ergänzt._

## Quellen

- Inventar: [F-486 in 01-inventar.md](../rewrite/01-inventar.md)
- Arbeitspaket: [T-017 in 06-arbeitsplan.md](../rewrite/06-arbeitsplan.md)
- Übersicht: [docs/index.md](../index.md)
