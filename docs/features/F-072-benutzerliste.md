---
id: F-072
title: Benutzerliste
status: geplant
modul: Authentifizierung, Berechtigungen, Benutzer und Rollen
paket: T-034
permission: offen
routes: ['/settings/users']
endpoints: ['listUsersRemote']
tables: ['users', 'user_roles', 'roles']
schemas: []
components: []
tests: []
updated: 2026-09-13
---

# F-072 — Benutzerliste

> **Status: geplant.** Diese Seite hält das Zielverhalten fest, wie es im
> Inventar des Vorgängersystems erhoben wurde. Sie wird mit Arbeitspaket
> **T-034** ausgefüllt und auf `status: umgesetzt` gesetzt.

## Zweck

Benutzerliste

## Erwartetes Verhalten

Suche über username/name/email (ILIKE), 25/Seite serverseitig, `createdAt DESC`, Spalten Benutzername/Name/Rollen/Status/Erstellt/Aktion, Rollen-Badges („keine"), Status-Badge, Zeile klickbar, Kartenliste < lg, Zähler „N Einträge", Leerzustand mit CTA

## Nutzersicht

_Wird mit T-034 ergänzt: was der Bediener sieht und tut, Schritt für Schritt._

## Berechtigungen

_Wird mit T-034 ergänzt._

## Zustände

| Zustand | Verhalten |
| --- | --- |
| Leer | _offen_ |
| Laden | _offen_ |
| Fehler | _offen_ |
| Keine Berechtigung | _offen_ |

## Technischer Bezug

| | |
| --- | --- |
| Routen | `/settings/users` |
| Endpoints | `listUsersRemote` |
| Tabellen | `users`, `user_roles`, `roles` |
| Schemata | _offen_ |
| Tests | _offen_ |

## Bekannte Grenzen

_Wird mit T-034 ergänzt._

## Quellen

- Inventar: [F-072 in 01-inventar.md](../rewrite/01-inventar.md)
- Arbeitspaket: [T-034 in 06-arbeitsplan.md](../rewrite/06-arbeitsplan.md)
- Übersicht: [docs/index.md](../index.md)
