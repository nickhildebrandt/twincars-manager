...
PDF generation/caching lives in `src/lib/server/services/pdf-service.ts` (built on `pdf-lib`) and is exposed through a **single global** `src/routes/pdfs.remote.ts` (not per-module). List views may only call the `getXPdfMetaRemote` queries — the bytes never travel through metadata calls. Viewing in the browser uses the browser's native PDF viewer via a Blob-URL iframe in `PdfViewer.svelte` (`pdfjs-dist` was removed as a dependency).
