...
Migrations do **not** run inside the request lifecycle. In production they run once via `scripts/migrate.js` before `node build` boots (see `Dockerfile` CMD); in dev use `pnpm db:migrate` or `pnpm db:push`. What `src/hooks.server.ts` _does_ run on the first request is `seedDefaults()` (idempotent — mail templates, ledger categories, number ranges); after the first hit it's a no-op. There is **no** in-process scheduler — recurring work (e.g. `autoSendDuePaymentReminders` in `reminder-service.ts`) is operator-triggered from the UI ("Jetzt prüfen"), designed so a future external cron can call the same path. `hooks.server.ts` also brute-force-throttles `POST /api/auth/sign-in/*` per client IP (`rate-limit.ts`, 10/min), throttles `/api/public/*` + the eBay compliance endpoint per Bearer-token prefix (fallback: IP; 120/min + 60 burst), and gates every non-public route behind an authenticated session.
...
- `.env` (gitignored) needs `DATABASE_URL` and `APP_SECRET` (HMAC secret for better-auth cookie signing). Template in `.env.example`. **Secrets at rest:** `$lib/server/crypto.ts` (AES-256-GCM, key = SHA-256 of `APP_ENCRYPTION_KEY`, falling back to `APP_SECRET` in dev) encrypts third-party credentials the app must read back — eBay OAuth tokens and the SMTP password (`v1:iv:tag:data` wire format; `decryptSecretIfNeeded` passes legacy plaintext rows through). The SMTP encrypt-and-upsert is centralized in `src/lib/server/services/smtp-settings-service.ts` (`upsertSmtpSettings`), used by both the setup and settings remotes. Normal business data is deliberately NOT encrypted at rest.
...
- **Four documented exceptions:** (1) `src/routes/api/auth/[...all]/+server.ts` is the better-auth catch-all — library plumbing for session cookies / CSRF that cannot be expressed as a remote function. (2) `src/routes/api/public/*` is the external read/write REST API (used-cars, services, tires, appointments, free-slots, orders, contact, company, posts), Bearer-token authenticated for third-party consumers. (3) `src/routes/api/ebay/account-deletion/+server.ts` is the eBay marketplace-account-deletion compliance endpoint — eBay's servers call it directly (challenge handshake + notifications), unauthenticated, whitelisted in `hooks.server.ts`, configured via `EBAY_VERIFICATION_TOKEN`. (4) `src/routes/api/ebay/oauth/callback/+server.ts` is the eBay OAuth redirect target (RuName "Auth Accepted URL") — browser round-trip plumbing, session-gated (NOT whitelisted), CSRF-protected via HMAC-signed `state`. Each endpoint splits the testable handler into `endpoint.ts` next to its `+server.ts`. App-internal data flow still uses remote functions exclusively; new exceptions need the same justification ("third-party plumbing or external consumer", not "convenience").
- Mutations use single-flight: `requested(listX, N).refreshAll()` on the server and `await mutate(...).updates(listX.withOverride(...))` on the client. Optimistic deletes/status-flips are the standard, not pessimistic refresh. **Both halves are required**: server-side `refreshAll()` alone is a no-op on the client — refreshed data only rides back for instances the client declared via `.updates(...)` (this shipped as a live bug in the absences UI).
- **Remote queries in event handlers need `.run()`**: a bare `await someQueryRemote(args)` only resolves in reactive contexts (top-level await, `$derived`, `$effect`); in a click handler or poll callback it throws or silently no-ops. This class produced 7+ real bugs (XRechnung/DATEV downloads, absences, hours pickers, import progress). Related: same-route detail-to-detail links need a keyed `[id]/+layout.svelte` (`{#key page.params.id}`; see `src/routes/invoices/[id]/+layout.svelte`) because detail pages freeze the id at init, and forms must clear `formDirty` BEFORE the post-save `goto` or the unsaved-changes confirm silently cancels the navigation.
- Pickers (entity-relation selection) live exclusively in `src/routes/pickers.remote.ts`. Never `<select>` for relationships, never client-side filtering of "the many" — always `SearchablePicker` + a `pickXRemote` query. Multi-select relations (e.g. work-order assignees) use `MultiSearchablePicker` (same server search + pagination, transactional "Übernehmen"). Creating a missing record goes through the **full-page creation flow** (the quick-create mini-forms are gone): the picker dialog shows a single "Neu anlegen" affordance in its header next to close (`createLabel` + `onCreateNew` props); the host snapshots its form as a JSON draft via `creationFlow.start(...)` (`src/lib/stores/creation-flow.svelte.ts`, sessionStorage-mirrored stack, 1 h stale drop); the leaves (`/customers/new`, `/vehicles/new`, `/employees/new`) `finish`/`cancel` back to `returnUrl`; the host restores the draft and auto-selects the created record via `pendingReturnFor`; `activeEntities()` is the cycle guard. Picker label formats are centralized in `src/lib/utils/picker-labels.ts`. Note that `pickEmployeesRemote` and `pickItemsRemote` also accept `orders` permission holders (`requireAnyPermission('employees'|'items', 'orders')`) so the shop floor can pick assignees and catalog items on work orders.
...
- Server-side: `handleValidationError` and `handleError` in `src/hooks.server.ts` produce the only message the user ever sees. Use `error(status, 'german message')` from `@sveltejs/kit` for curated 4xx. `handleValidationError` translates field keys through the `FIELD_LABELS` map in `hooks.server.ts` (~100 German labels; array indices render as "(Position N)"); add a German label there for every new schema field key.
...
- **Action buttons are never disabled for missing/invalid input.** Only `busy.active` and true mode gates may disable a button. Validation is click-time: mark all fields touched, render a German error summary plus field errors, return. Forms carry `novalidate`.
...
Tabs: `TabGroup.svelte` is the one app-wide tab implementation (tabs-lift radio pattern, `?tab=` deep links, nav mode when every tab has an `href` — drives the settings layout). Content switching = TabGroup; mutually exclusive list filters (Alle/Archiv) stay simple filter tabs; no nested tab groups anywhere (CONTRIBUTING §7).
...
Archiving is the soft-delete path for customers and vehicles: Archiv tab on the list (hidden by default; ignores kind filters), Archivieren/Reaktivieren via `ConfirmDialog` on the detail, delete guards that refuse with German counts when links exist, archived records excluded from pickers and global search (CONTRIBUTING §8).

### PDFs

PDF generation/caching lives in `src/lib/server/services/pdf-service.ts` (built on `pdf-lib`) and is exposed through a **single global** `src/routes/pdfs.remote.ts` (not per-module). List views may only call the `getXPdfMetaRemote` queries — the bytes never travel through metadata calls. Viewing in the browser uses the browser's native PDF viewer via a Blob-URL iframe in `PdfViewer.svelte` (`pdfjs-dist` was removed as a dependency). Document PDFs are persisted at creation time; `cancelInvoice` persists the Storno PDF the same way at cancel time (the view path reads only from the cache and would otherwise 404). Renders are byte-deterministic and guarded by a visual-regression suite (`pdf-visual.test.ts`, committed snapshots in `__pdf_snapshots__/`; refresh intended layout changes with `PDF_SNAPSHOTS=update`).

### Adding a new module

All modules are now end-to-end implemented (customers, vehicles, inventory, suppliers, employees, items, offers, invoices, orders (Aufträge, Kanban work orders with completion-to-invoice), reminders (nav label "Offene Rechnungen"; the artifact wording stays "Zahlungserinnerung"), sent, ledger, sales-ledger, calendar, mailings, settings, plus time tracking under `hours`, tire catalog/reminders under `tires`, `tire-storage`, and `posts` (Aktuelle Informationen / news, with a paginated public API)) — there are no remaining `ComingSoon` stubs. The settings area (`/settings/*`) is flat — one permission-filtered nav-mode `TabGroup` bar (`src/routes/settings/+layout.svelte`) over own routes: mail templates at `/settings/mail`, reminders at `/settings/reminders`, SMTP (incl. Testversand) at `/settings/smtp` (legacy `/settings?tab=` deep links redirect); the KFZ-Kaufmann import lives at `/settings/import` and the eBay connection + listing import at `/settings/ebay` — there is no top-level `/import` route anymore. Note the **payroll** and **special-payments** modules were removed: payroll was replaced by time tracking (`time-entry-service.ts`, `workshop-hours-service.ts`, route `/hours`). **customers** and **vehicles** remain the canonical templates — they're the cleanest illustration of the list / detail / form + form-component + co-located test pattern.
...
3. `src/routes/<x>/+page.svelte`: list per CONTRIBUTING §5, i.e. only-set-key `queryArgs`, `await untrack(() => listXRemote(queryArgs))` seed, read via `$derived.by(() => listXRemote(queryArgs).current ?? lastResult)` (never memoize the query proxy), one `$effect` syncing `lastResult` + errors.
...
- TypeScript only (`.ts` / `.svelte`). Svelte 5 + Runes is binding — no `export let`, no `$:`, no `on:event` directives. `svelte` is pinned exact (5.56.4) and must stay at or above 5.56.x: 5.55.5 broke every 2nd+ args change on list pages in production builds (stale-list framework bug).
